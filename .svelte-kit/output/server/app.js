...
const U5BchapterIdu5D = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($store[mangaId] !== void 0) {
    ...
    id = chapterInfo[chapterId - 1].chapterId;
    const newLink = chapterLink.replace(/\//g, ",");
    axios.get(`https://server-mv.herokuapp.com/chapter/${source}/${mangaId}/${newLink}`).then((res) => {
      chapter = res.data;
    }).catch((err) => {
      console.log(err);
    });
    ...
    const newLink = link.replace(/\//g, ",");
    axios.get(`https://server-mv.herokuapp.com/manga/${source}/${mangaId}/${newLink}`).then((res) => {
      store.update((currentData) => {
        return {
      ...
      id = chapterInfo[chapterId - 1].chapterId;
      const newChapterLink = chapterLink.replace(/\//g, ",");
      axios.get(`https://server-mv.herokuapp.com/chapter/${source}/${mangaId}/${newChapterLink}`).then((res) => {
        chapter = res.data;
      }).catch((err) => {
        console.log(err);
      });
